...
const Board = () => {
  ...
  const columnDefs = useMemo<ColDef<BoardRow>[]>(
    () => [
      { headerName: 'Name', field: 'ticker', pinned: 'left', width: 104, cellRenderer: NameCell, sortable: true, headerTooltip: 'Click a name to open it on the Map' },
      { headerName: 'Price', field: 'spot', flex: 1, minWidth: 112, cellRenderer: SpotCell, comparator: (a: number, b: number) => a - b, headerTooltip: "The last price and today's change" },
      {
      ...
      { headerName: 'Hedging today', field: 'regime', flex: 1, minWidth: 128, cellRenderer: RegimeCell, headerTooltip: 'Which way dealer hedging leans right now: absorbing moves (price above the flip) or amplifying them (price below it)' },
      {
      ...
      { headerName: 'Supreme', field: 'supreme', flex: 0.6, minWidth: 72, cellRenderer: SupremeCell, headerTooltip: 'The heaviest strike on the whole book — the one the Map marks in magenta' },
      { headerName: 'Net GEX', field: 'netGex', flex: 0.8, minWidth: 90, cellRenderer: NetCell, comparator: (a: number, b: number) => Math.abs(a) - Math.abs(b), headerTooltip: 'All the hedging on the name added up, per 1% move: positive absorbs moves, negative amplifies them' },
      { headerName: 'Expires today', field: 'bellShare', flex: 0.9, minWidth: 100, cellRenderer: BellCell, headerTooltip: "The share of the name's hedging sitting in contracts that expire at 4:00 today — it is gone at the close, and the walls it made go with it" },
      { headerName: 'Crossed', field: 'crossings', flex: 0.6, minWidth: 72, cellRenderer: CrossCell, headerTooltip: 'How many times price has crossed the flip today — the more it has, the less either side of it means' },
      { headerName: 'Today so far', field: 'spark', flex: 0.8, minWidth: 90, cellRenderer: SparkCell, cellDataType: false, sortable: false, headerTooltip: 'The price path since the open' },
      {
        headerName: 'Yours',
        field: 'yours',
        flex: 0.6,
